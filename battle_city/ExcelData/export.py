...
class SheetInterpreter:
    ...
    def __init__(self, xls_file_path, sheet_name, export_path):
    ...
    def Interpreter(self) :
        ...
        self._output.append("package Base;\n")
        ...
        # 将PB转换成py格式
        try :
            command = "protoc --python_out=./Cache -I=" + self._export_path + " " + self._export_path + self._pb_file_name
            os.system(command)
        except BaseException, e :
            print "protoc failed!"
            raise
    ...
    def _Write2File(self) :
        """输出到文件"""
        pb_file = open(self._export_path + self._pb_file_name, "w+")
...
class DataParser:
    ...
    def Parse(self) :
        ...
        for sheet in self._sheets:
            ...
            data = data + item_array.SerializeToString()
    ...
    def _WriteReadableData2File(self, data) :
        file_name = "Cache/" + self._sheet_name + ".txt"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()
...
if __name__ == '__main__' :
    ...
    try :
        tool = SheetInterpreter(xls_file_path, sheet_name, proto_path)
        tool.Interpreter()
    except BaseException, e :
    ...
    try :
        parser = DataParser(xls_file_path, sheet_name, data_path)
        parser.Parse()
    except BaseException, e :
        print "Parse Failed!!!"
        print e
